...
const getAllFavorites = async(req,res)=>{
    try{
       const favorites = await Favorite.find({
        userId: req.user.id
       }).populate("songId") // ye find method se hum saare favorites ko fetch karenge aur populate method se hum songId ko populate karenge taki hume song ka pura data mil jaye
...
const addFavorite = async(req,res)=>{
    try{
        ...
        });  // ye create method se hum ek naya favorite create karenge aur usko database me save karenge
...
const deleteFavorite = async(req,res)=>{
    try{
        const favorite = await Favorite.findByIdAndDelete({
            _id:req.params.id,
            userId:req.user.id,
        }); // ye findByIdAndDelete method se hum ek favorite ko delete karenge aur usko database se delete kar denge, yaha pe req.params.id se hum id ko fetch karenge jo url me aayega
        if(!favorite){
